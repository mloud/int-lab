...
const TextCompressionStepByStep: React.FC<{ onBack: () => void }> = ({ onBack }) => {
  const [inputText, setInputText] = useState('AHOJ');
  const [step, setStep] = useState<1 | 2 | 3 | 4>(1);

  // Bezpečný vstup - max 30 znaků, pouze písmena a mezery
    ...
    <div className="w-full max-w-4xl mx-auto flex flex-col gap-6 animate-in fade-in duration-500 pb-10 px-4">
      ...
      <div className={`bg-white p-8 rounded-[2.5rem] shadow-lg border-2 transition-all duration-500 ${step >= 1 ? 'border-blue-200 opacity-100' : 'opacity-50 pointer-events-none'}`}>
        <div className="flex items-center gap-4 mb-6">
          ...
          <div>
            <h2 className="text-xl font-black text-gray-800 uppercase tracking-tight">Vstupní text</h2>
            <p className="text-sm font-bold text-gray-400">Napiš větu s opakujícími se slovy (např. PETR JE PETR, max 30 písmen bez diakritiky).</p>
          </div>
        </div>
        ...
        <div className={`bg-white p-8 rounded-[2.5rem] shadow-lg border-2 transition-all duration-500 animate-in fade-in slide-in-from-top-10 ${step >= 2 ? 'border-indigo-200 opacity-100' : 'opacity-50'}`}>
          <div className="flex items-center justify-between mb-6">
            <div className="flex items-center gap-4">
              ...
              <div>
                <h2 className="text-xl font-black text-gray-800 uppercase tracking-tight">Kódování znaků (ASCII)</h2>
                <p className="text-sm font-bold text-gray-400 max-w-xl">
                  Počítač nerozumí písmenům. Každé písmeno proto musí převést na číslo podle dohodnuté tabulky (tzv. ASCII tabulka).
                </p>
              </div>
            </div>
            {step === 2 && (
        ...
        <div className={`bg-white p-8 rounded-[2.5rem] shadow-lg border-2 transition-all duration-500 animate-in fade-in slide-in-from-top-10 ${step >= 3 ? 'border-emerald-200 opacity-100' : 'opacity-50'}`}>
          <div className="flex items-center justify-between mb-6">
            <div className="flex items-center gap-4">
              ...
              <div>
                <h2 className="text-xl font-black text-gray-800 uppercase tracking-tight">Převod čísel do binární podoby</h2>
                <p className="text-sm font-bold text-gray-400 max-w-xl">
                  Čísla (desítková) se nyní převedou na "jedničky a nuly" (bity). Každé písmeno zabere přesně 8 bitů (1 byte).
                </p>
              </div>
            </div>
            {step === 3 && (
          ...
          <div className="flex flex-col gap-2 bg-gray-900 p-4 md:p-6 rounded-[2rem] shadow-inner font-mono text-emerald-400 max-h-64 overflow-y-auto">
            ...
            <div className="mt-4 pt-4 border-t border-gray-800 flex items-center justify-between sticky bottom-0 bg-gray-900/90 backdrop-blur-sm pb-2">
               <span className="text-xs font-bold text-gray-500 uppercase tracking-widest">Celková velikost textu:</span>
               <span className="px-3 py-1 bg-gray-800 rounded-lg text-white font-black text-sm">{characters.length} Bytů ({characters.length * 8} bitů)</span>
            </div>
          </div>
        </div>
      )}
      ...
      {step >= 4 && (
        <div className="bg-gradient-to-br from-orange-50 to-amber-50 p-8 rounded-[2.5rem] shadow-xl border-4 border-orange-200 animate-in fade-in slide-in-from-top-10">
          <div className="flex items-center gap-4 mb-6">
            ...
            <div>
              ...
              <p className="text-sm font-bold text-gray-500 max-w-2xl">
                ZIP v textu hledá slova, která se opakují. Namísto jejich opětovného ukládání vloží pouze odkaz na jejich první výskyt.
              </p>
            </div>
          </div>
          ...
          <div className="bg-white p-6 rounded-3xl shadow-sm border border-orange-100 mb-6">
            ...
            <div className="flex flex-col md:flex-row gap-6 items-stretch">
              ...
              <div className="flex-1 w-full bg-gray-50 p-6 rounded-2xl border border-gray-200 text-center relative flex flex-col justify-center">
                <div className="text-xs font-black text-gray-400 uppercase tracking-widest mb-4">Původní text</div>
                ...
                <div className="mt-4 text-[10px] font-bold text-gray-400 uppercase">
                  Zabere <span className="text-gray-600 font-black">{originalSize} Bytů</span>
                </div>
              </div>
              ...
              <div className={`flex-1 w-full p-6 rounded-2xl border-4 text-center relative shadow-inner flex flex-col justify-center ${isCompressed ? 'bg-orange-100 border-orange-300' : 'bg-gray-100 border-gray-200'}`}>
                ...
                <div className="mt-auto pt-4 border-t-2 border-black/5">
                  <div className={`text-[11px] font-black uppercase ${isCompressed ? 'text-orange-700' : 'text-gray-500'}`}>
                    Výsledek: <span className={`${isCompressed ? 'text-emerald-600' : 'text-gray-700'} text-sm`}>
                      {isCompressed ? `Zabere jen ${compressedSize} Bytů!` : `Velikost se nezměnila (${compressedSize} Bytů)`}
                    </span>
                  </div>
                </div>
              </div>
          ...
          <div className="bg-white/60 p-5 rounded-2xl border border-orange-100/50">
            <h4 className="text-sm font-black text-gray-800 uppercase mb-2">Shrnutí pro ZŠ:</h4>
            <p className="text-sm font-bold text-gray-600 leading-relaxed">
              Když zadáš text <strong className="text-gray-800">bez opakujících se slov</strong>, ZIP nedokáže nic ušetřit a soubor bude stejně velký. 
              Pokud se ale nějaké slovo opakuje, ZIP ho uloží jen poprvé a na další místa zapíše malou značku (např. <strong className="text-orange-600">#1</strong>), která počítači řekne: <em>"Tady doplň to první uložené slovo"</em>. 
              Díky tomu se soubor zmenší!
            </p>
          </div>
...
// Pomocná komponenta pro šipku v textu (jelikož Lucide ArrowRight tam neseděla)
...
export default TextCompressionStepByStep;
